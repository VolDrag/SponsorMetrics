...
const ContractsPage = () => {
  ...
  return (
    <DashboardLayout>
      ...
      <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Type full legal name to sign" className="mt-4 w-full max-w-md rounded border px-3 py-2" />
        ...
        {rows.map((row) => (
          <article key={row._id} className="rounded-xl border bg-white p-4">
            ...
            <p className="text-sm text-slate-500">
              {(row.organizerId?.organizationName || row.organizerId?.name || 'Organizer')} × {(row.sponsorId?.organizationName || row.sponsorId?.name || 'Sponsor')}
            </p>
            ...
            <div className="mt-3 flex flex-wrap gap-3">
              ...
              <Link to="/payments" className="text-sm text-slate-600">Go to payments</Link>
            </div>
          </article>
        ))}
        {!rows.length && <p className="rounded-xl border bg-white p-8 text-center text-sm text-slate-500">No contracts yet. They appear when a proposal is accepted.</p>}
      </div>
    </DashboardLayout>
  );
};
