...
const ProposalReview = ({ backTo, backLabel }) => {
  ...
  const lastActionRole =
    proposal?.lastActionBy?.role ||
    pending?.role ||
    (openForAction ? 'organizer' : null);
  const waitingOnOtherParty = Boolean(openForAction && lastActionRole && lastActionRole === user?.role);
  const waitingForLabel = lastActionRole === 'organizer' ? 'Sponsor' : 'Organizer';
    ...
    <DashboardLayout>
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-6 flex flex-wrap items-center gap-3">
          ...
          {user?.role === 'organizer' && proposal.eventId?._id && (
            <MarketingAdvicePanel eventId={proposal.eventId._id} />
          )}
        ...
        <div className="mb-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="grid gap-4 text-sm md:grid-cols-2">
            <p>
              ...
              {proposal.organizerId?._id ? (
                <Link to={`/profile/${proposal.organizerId._id}`} className="text-amber-700 hover:underline">
                  {proposal.organizerId?.organizationName || proposal.organizerId?.name}
                </Link>
              ) : (
                proposal.organizerId?.organizationName || proposal.organizerId?.name
              )}
            ...
            <p>
              ...
              {proposal.sponsorId?._id ? (
                <Link to={`/profile/${proposal.sponsorId._id}`} className="text-amber-700 hover:underline">
        ...
        {openForAction && (
          <div className="space-y-4">
            {/* BUGFIX: prevent self-acceptance of own offer — Module 2 Feature 2 */}
            {waitingOnOtherParty ? (
              <span className="inline-flex items-center rounded-full border border-amber-200 bg-amber-50 px-3 py-1.5 text-sm font-medium text-amber-800">
                Waiting for {waitingForLabel} to respond
              </span>
            ) : showCounter ? (
              <CounterOfferForm
