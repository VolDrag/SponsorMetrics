...
// Rafi
  static async discoverEventsForSponsor(sponsorId, filters = {}) {
    ...
    let events = await Event.find(query).populate('organizerId', 'name organizationName avgReliability avgCommunication reviewCount credibilityScore orgVerified');
    ...
    const matchedEvents = events.map(event => {
      ...
      const credPoints = Math.round((sponsor.credibilityScore || 0) * 3); // module 1, feature 4 (Rafi) — fix: guard against undefined credibilityScore
      score += credPoints;
