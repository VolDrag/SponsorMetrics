...
const { recalculateForSponsor } = require('./budget.controller'); // MODULE 3 | Feature 3
const { screenProposal, holdMessage } = require('../services/fraud.service'); // MODULE 4 | Feature 3
...
exports.createProposal = async (req, res) => {
  try {
      ...
      proposedBudget:
        proposedBudget !== undefined && proposedBudget !== null && proposedBudget !== ''
          ...
          : tier
            ? tier.price
            : 0,
      formatType: tier?.formatType || 'other',
    };
    ...
    if (shouldSend) {
      ...
      if (screening.flags.length) {
        const held = await Proposal.create({
          ...
          fraudRiskScore: screening.fraudRiskScore,
        });
        const populatedHeld = await Proposal.findById(held._id).populate(POPULATE);
        return res.status(400).json({
          success: false,
          message: holdMessage(screening.flags),
          data: populatedHeld,
          fraudFlags: screening.flags,
          fraudRiskScore: screening.fraudRiskScore,
        });
    ...
    const proposal = await Proposal.create({
      ...
      lastActionBy: shouldSend
        ? { userId: req.user._id, role: 'organizer' }
        : undefined,
    });
...
exports.getSponsors = async (req, res) => {
  try {
    const sponsors = await User.find({ role: 'sponsor', isActive: { $ne: false } })
      .select('name email organizationName industry budgetTier isVerified credibilityScore avgReliability avgCommunication reviewCount')
...
exports.updateProposal = async (req, res) => {
  try {
    ...
    if (updates.selectedTierId) {
      ...
      if (tier?.formatType) updates.formatType = tier.formatType;
...
exports.sendProposal = async (req, res) => {
  try {
    ...
    if (screening.flags.length) {
      await proposal.save();
      const populatedHeld = await Proposal.findById(proposal._id).populate(POPULATE);
      return res.status(400).json({
        success: false,
        message: holdMessage(screening.flags),
        data: populatedHeld,
    ...
    proposal.lastActionBy = { userId: req.user._id, role: 'organizer' };
    if (proposal.selectedTierId) {
      const tier = await SponsorshipTier.findById(proposal.selectedTierId);
      // ===== MODULE 3 FEATURE 4: A/B Experiment Tracker — START =====
      if (tier?.formatType) proposal.formatType = tier.formatType;
      // ===== MODULE 3 FEATURE 4: A/B Experiment Tracker — END =====
    }
    await proposal.save();
...
// BUGFIX: prevent self-acceptance of own offer — Module 2 Feature 2
const resolveLastActionRole = (proposal) => {
  if (proposal.lastActionBy?.role) return proposal.lastActionBy.role;
  const pending = latestPendingCounter(proposal);
  if (pending?.role) return pending.role;
  const lastOffer = (proposal.counterOffers || [])[(proposal.counterOffers || []).length - 1];
  if (lastOffer?.role) return lastOffer.role;
  if (['sent', 'viewed', 'negotiation'].includes(proposal.status)) return 'organizer';
  return null;
};

// BUGFIX: prevent self-acceptance of own offer — Module 2 Feature 2
const assertOtherPartyCanAct = (proposal, user) => {
  const lastRole = resolveLastActionRole(proposal);
  if (lastRole && user.role === lastRole) {
    return {
      statusCode: 403,
      message: "You already sent the last offer — waiting for the other party's response.",
    };
  }
  return null;
};
...
exports.counterOffer = async (req, res) => {
  try {
    ...
    // BUGFIX: prevent self-acceptance of own offer — Module 2 Feature 2
    const turnError = assertOtherPartyCanAct(proposal, req.user);
    if (turnError) {
      return res.status(turnError.statusCode).json({ success: false, message: turnError.message });
    }
    ...
    const actorRole = req.user.role === 'organizer' ? 'organizer' : 'sponsor';
    ...
    // BUGFIX: prevent self-acceptance of own offer — Module 2 Feature 2
    proposal.lastActionBy = { userId: req.user._id, role: actorRole };
    await proposal.save();
...
exports.acceptProposal = async (req, res) => {
  try {
    ...
    if (proposal.sponsorId) {
      await recalculateForSponsor(proposal.sponsorId).catch((err) => {
        console.error('Budget pacing recalculation failed:', err.message);
      });
    }
