// backend/src/controllers/analyzerController.js
  ...
  try {
    ...
    if (!proposalData || Object.keys(proposalData).length === 0) {
      return res.status(400).json({
        success: false,
        message: 'Proposal data is required',
      });
    }
    ...
    if (proposalData.eventId) {
      await Proposal.findOneAndUpdate(
        { eventId: proposalData.eventId },
        {
          ...
          analyzedAt: new Date(),
          ...(proposalData.rawBulletPoints && { rawBulletPoints: proposalData.rawBulletPoints }),
          ...(proposalData.aiGeneratedText && { aiGeneratedText: proposalData.aiGeneratedText }),
        },
  ...
  try {
    ...
    if (!proposal) {
      return res.status(404).json({
        success: false,
        message: 'No analysis found for this event',
      });
    }
